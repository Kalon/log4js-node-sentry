...
function sentryAppender(client, layout, level) {
    ...
    return function(logEvent) {
        ...
        if (requests.length) parseRequest(requests[0], kwargs)
...
function simpleLayout(loggingEvent) {
    return util.format.apply(util, loggingEvent.data)
}
...
function parseRequest(req, kwargs) {
	raven.parsers.parseRequest(req, kwargs);
	kwargs.user = { ip_address: req.ip };
	if (null != req.user) kwargs.user.id = req.user.id;
}
